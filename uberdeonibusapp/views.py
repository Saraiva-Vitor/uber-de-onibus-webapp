...
def cadastro(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            call_command(
                "createuser", user.email, form.cleaned_data["password"], user.first_name
            )
...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.error(
                request, "Credenciais inválidas. Verifique seu email/CPF e senha."
            )
...
def pesquisa(request):
    ...
    for rota in rotas:
        ...
        preco_tipo_onibus = (
            rota.bus.tipo_onibus.preco if rota.bus and rota.bus.tipo_onibus else 0
        )
        ...
        return render(
            ...
            "pesquisa.html",
            {
                "rotas_com_soma": rotas_com_soma,
                "locais": locais,
                "rotas": rotas,
                "dinamico": dinamico,
                "nome_origem": rotas[0].origin.name,
